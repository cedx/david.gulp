...
Checker.prototype.parseManifest=function(file) {
  ...
  var manifest=null;
  try { manifest=JSON.parse(file.contents); }
...
Checker.prototype._getDependencies=function(getter, manifest) {
  ...
  var getDeps=Promise.denodeify(getter);
  var promises=[
    getDeps(manifest, options),
    getDeps(manifest, util._extend(util._extend({}, options), { dev: true })),
    getDeps(manifest, util._extend(util._extend({}, options), { optional: true }))
  ];
    ...
    function(deps) {
      if(self._options.update) {
        for(var type in deps) {
          var version=(self._options.unstable ? 'latest' : 'stable');
          for(var name in deps[type]) manifest[type][name]='^'+deps[type][name][version];
        }
